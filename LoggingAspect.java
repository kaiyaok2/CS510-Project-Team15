...
public class LoggingAspect {
    ...
    public Object logMethodVariables(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (calledOnInstance) {
            allLogs.add("Instance Initial: " + CustomToStringConverter.customToString(instance));
        }

        for (int i = 0; i < args.length; i++) {
            originalParams.add(i);
            Object currentValue = args[i];
            allLogs.add(("Arg" + i + " initial state: " + CustomToStringConverter.customToString(currentValue)));
        }
        ...
        try {
            ...
            allLogs.add("Exception thrown: " + CustomToStringConverter.customToString(e));
        ...
        if (calledOnInstance) {
            allLogs.add("Instance Final: " + CustomToStringConverter.customToString(instance));
        }

        for (int j = 0; j < args.length; j++) {
            if (originalParams.contains(j)) {
                Object currentValue = args[j];
                allLogs.add(("Arg" + j + " final state: " + CustomToStringConverter.customToString(currentValue)));
            }
        }


        if (result != null) {
            allLogs.add(("Return value: " + CustomToStringConverter.customToString(result)));
        }
    ...
    public static Set<Class<?>> getAllClasses() {
        ...
        if (fullPackage.contains("org.apache.commons.codec") || fullPackage.contains("me.lemire") || fullPackage.contains("com.kamikaze")) {
            return new HashSet<>();
        }
